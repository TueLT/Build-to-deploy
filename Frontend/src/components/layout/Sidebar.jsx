    ...
    <>
      ...
      <aside className={`app-sidebar ${open ? 'open' : ''}`}>
        ...
        <nav className="sidebar-nav">
          ...
          {nav.map(([path, icon, label]) => (
            <NavLink key={path} to={`/${path}`} onClick={onClose} className={({ isActive }) => `side-link ${label === 'AI Assistant' ? 'assistant-link' : ''} ${isActive ? 'active' : ''}`}>
              <i className={`bi ${icon}`} /><span>{label}</span>{label === 'Chats' && <span className="side-count">4</span>}{label === 'AI Assistant' && <span className="new-pill">New</span>}
            </NavLink>
          ))}
        </nav>
        <div className="sidebar-bottom">
          ...
          <NavLink to="/profile" className="user-mini"><span className="avatar-photo">AR</span><span><strong>Alex Rivera</strong><small>Product Designer</small></span><i className="bi bi-three-dots ms-auto" /></NavLink>
        </div>
      </aside>
    </>
  )
