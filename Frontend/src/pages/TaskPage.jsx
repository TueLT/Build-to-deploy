...
export default function TaskPage() {
  ...
  const accept = (task) => updateTaskStatus(token, task.id, 'pending').then(refresh)
  const dismiss = (task) => updateTaskStatus(token, task.id, 'dismissed').then(refresh)
  const complete = (task) => updateTaskStatus(token, task.id, 'completed').then(refresh)
  const remove = (task) => deleteTask(token, task.id).then(refresh)
  ...
  return <div className="page-container">
    ...
    <section className="content-card"><div className="card-toolbar"><div><h3>All tasks</h3><span>{mainTasks.length} tasks across your conversations</span></div><div className="toolbar-actions"><div className="mini-search"><i className="bi bi-search"/><input placeholder="Search tasks"/></div><button className="btn btn-light"><i className="bi bi-filter me-2"/>Filter</button></div></div>{loading ? <p className="text-muted small p-3 mb-0">Loading...</p> : <TaskTable tasks={mainTasks} onComplete={complete} onDelete={remove}/>}</section>
