...
- **Trích xuất Task cá nhân từ hội thoại**: cùng panel AI → **Extract tasks** — AI chỉ đọc phạm vi message người dùng chọn và tạo candidate riêng của người đó. `conversation_id`/`source_message_ids` chỉ là provenance; Personal Agent không tự biến candidate thành work item dùng chung của Agent Workspace. Người dùng bấm **Accept**/**Dismiss** để xác nhận.
- **Task theo Workspace channel**: cam kết được phát hiện trong channel Product Delivery được bind rõ vào đúng `agent_workspace_id` + `conversation_id`. Trong Workspace Agent, Member mặc định xem task của mình nhưng có thể chọn channel mình đang tham gia để truy vấn tổng hợp toàn bộ task của channel; quyền sửa vẫn chỉ áp dụng cho task của mình, còn assign/review team task thuộc Lead.
